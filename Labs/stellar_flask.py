...
class Stellar_pop_lab:
    def __init__(self, server, requests_pathname_prefix, routes_pathname_prefix, app_name=None):
        print("=== Stellar_pop_lab: starting __init__ ===")
        try:
            self.app = dash.Dash(
            ...
            print("=== Stellar_pop_lab: dash.Dash created OK ===")
        except Exception:
            print("=== Stellar_pop_lab: FAILED creating dash.Dash ===")
            traceback.print_exc()
            raise
        ...
        try:
            ...
            print("=== Stellar_pop_lab: load_data OK ===")
        except Exception:
            print("=== Stellar_pop_lab: FAILED load_data ===")
            traceback.print_exc()
            raise

        try:
            self.pregenerate_model()
            print("=== Stellar_pop_lab: pregenerate_model OK ===")
        except Exception:
            print("=== Stellar_pop_lab: FAILED pregenerate_model ===")
            traceback.print_exc()
            raise

        try:
            self.generate_model()
            print("=== Stellar_pop_lab: generate_model OK ===")
        except Exception:
            print("=== Stellar_pop_lab: FAILED generate_model ===")
            traceback.print_exc()
            raise

        try:
            self.setup_layout()
            print("=== Stellar_pop_lab: setup_layout OK ===")
        except Exception:
            print("=== Stellar_pop_lab: FAILED setup_layout ===")
            traceback.print_exc()
            raise

        try:
            self.setup_callbacks()
            print("=== Stellar_pop_lab: setup_callbacks OK ===")
        except Exception:
            print("=== Stellar_pop_lab: FAILED setup_callbacks ===")
            traceback.print_exc()
            raise

        print("=== Stellar_pop_lab: __init__ complete ===")
    ...
    def setup_callbacks(self):
        @self.app.callback(
            ...
            State("active-dataset", "data"),   # single State, not wrapped in list
        ...
        def update_app(*args):
            ...
            # args layout: 6 sliders | init-trigger | N buttons | 1 State
            slider_args         = args[:6]
            # args[6] = init-trigger n_intervals (skip)
            btn_args            = args[7:7 + n_datasets]
            current_dataset_key = args[7 + n_datasets]   # State
            ...
            new_dataset_key = current_dataset_key or self.initial_dataset
            if ctx.triggered:
    ...
    def create_main_plot(self):
        ...
        if not model_valid:
            ...
            fig.add_trace(go.Scatter(x=self.model_spectrum[:, 0] / 1e4,
                                     y=self.model_spectrum[:, 1] / 1e-18,
                                     mode='lines', line=dict(color='firebrick', shape='hv'),
                                     name='Model', showlegend=True))

        fig.add_trace(go.Scatter(x=self.data_wave, y=self.data_flux / 1e-18,
                                 mode='lines', line=dict(color='black', shape='hv'),
                                 name='Observations', showlegend=True))

        if model_valid:
            for line_name, (obs_wave, color) in self.get_emission_lines().items():
    ...
    def create_sfh_plot(self):
        ...
        try:
            age_universe, sfr = self.calculate_sfh()
            fig.add_trace(go.Scatter(x=age_universe, y=sfr, mode='lines',
            ...
            fig.update_layout(title="Star Formation History",
    ...
    def create_sfr_mass_plot(self):
        ...
        try:
            ...
            fig.add_trace(go.Scatter(
                x=[self.Mass], y=[current_sfr], mode='markers',
